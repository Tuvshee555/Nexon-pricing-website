...
export async function POST(request: Request) {
  ...
  if (!isJson && !isTxt) {
    return NextResponse.json({ error: "File must be .json or .txt" }, { status: 400 });
  }

  if (file.size > MAX_UPLOAD_BYTES) {
    return NextResponse.json({ error: "File must be 200 KB or smaller" }, { status: 400 });
  }
  ...
  if (isJson) {
    ...
    // Plain text — wrap in a type envelope so appendKnowledgeSection knows how to render it
    parsed = { type: "plaintext", content: rawText };
  }
